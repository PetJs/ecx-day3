...
describe('TodoApp.addTask', () => {
  ...
  test('adds a new task when input has text', () => {
    mockInput.value = 'Open your Laptop';
    app.addTask();

    expect(app.tasks.length).toBe(1);
    expect(app.tasks[0].text).toBe('Open your Laptop');
  ...
  test('clears input and disables button after adding', () => {
    mockInput.value = 'Take out trash';
    app.addTask();    
    expect(mockInput.value).toBe('');
    expect(mockAddBtn.disabled).toBe(true);
  });
});
